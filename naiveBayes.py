  ...
  def trainAndTune(self, trainingData, trainingLabels, validationData, validationLabels, kgrid):
    ...
    for i in range(0, len(trainingData)):
      ...
      for pixel, value in datum.items():
        f[(pixel, label)] = f[(pixel, label)] + 1
        if (value == 1):
          cp[(pixel, label)] = cp[(pixel, label)] + 1
    
    highest = 0;
    
    # Loop over all kgrid values for smoothing
    for k in kgrid:
      ...
      for pixelANDlabel, value in cp.items():
        kcp[pixelANDlabel] = value
        
      for pixelANDlabel, value in f.items():
        kf[pixelANDlabel] = value
      
      # We add the k value
      for pixel in self.features:
        for label in self.legalLabels:
          kcp[(pixel, label)] = kcp[(pixel, label)] + k
          kf[(pixel, label)] = kf[(pixel, label)] + k 
          
      for i, j in kcp.items():
        kcp[i] = float(j) / kf[i]
  ...
  def calculateLogJointProbabilities(self, datum):
    ...
    for i in self.legalLabels:
			...
			for pixel, value in datum.items():
				...
				else: # if the value is not 0, we add
